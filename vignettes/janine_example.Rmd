...
```{r janine}
out <- janine(X, n_blocks = nbBlock, # alpha = 0.1,
              control_optim = list(n_cores = 1),
              control_penalties = list(min_ratio = 0.05))
```

In this setting, the criteria looks good but probably need to be adapted cause they choose a too sparse model

```{r criteria_plot}
plot(out$criteria$penalty, -2*out$criteria$loglik, type = "b", xlab = 'iteration', ylab = 'criteria', log = "x")
lines(out$criteria$penalty, out$criteria$BIC, col = "red")
lines(out$criteria$penalty, out$criteria$EBIC, col = "blue")
legend('bottomright', bty = "n", c("-2Loglik", "BIC", "EBIC"), col = c("black", "red", "blue"), lty = c(1,1,1))
abline(v = c(out$criteria$penalty[which.min(out$criteria$EBIC)],out$criteria$penalty[which.min(out$criteria$BIC)]), lty = "dotted")
```
...
```{r model selection}
model_EBIC  <- out$models[[which.min(out$criteria$BIC)]]
