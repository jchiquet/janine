...
From this networjk we build a signed precision matrix 
...
\begin{equation}
  \mathbf{W} = \frac{(1 - \hat{\mathbb{P}}(i \leftrightarrow j)) }{1 - \mathrm{density}(\mathcal{G})},
\end{equation
where $\mathcal{G}$ is the current estimate of the network and $\hat{\mathbb{P}}(i \leftrightarrow j))$ is the probability of connection between any dyad under the fitted SBM.

The overall amount of sparsity is tuned by the argument `penalty`, and the number of block in the SBM is fixed by argument `n_blocks`
